...
import baldPenguin from "./Images/baldpenguin.png";
import timeSheep from "./Images/timeSheep.png";
import baldTime from "./Images/baldTime.png";
...
const sheepCatalogueText: Record<SheepType, string[]> = {
  ...
  penguin: [
    "fixer magazine fanatic",
    "maybe resistant to the cold?",
...
const sheepConfig: Record<string, SheepConfigEntry> = {
  normal: {
    image: sheep,
...
const penguinLines = [
  ...
  "i am not a sheep",
  "do NOT shear me",
  "fixer magazine issue 11 is peak",
  "matthias!!!!!!!!!!!!!",
];
...
export default function App() {
  ...
  const [timeRipple, setTimeRipple] = useState(false);
  ...
  const [woolMarket, setWoolMarket] = useState({
    ...
    penguin: sheepConfig.penguin.woolValue,
  ...
  const generateMarketPrices = () => {
    setWoolMarket({
      ...
      penguin: Math.floor(
        sheepConfig.penguin.woolValue * ((0.7 + Math.random() * 0.6) * rep),
      ),
      time: Math.floor(
        sheepConfig.time.woolValue * ((0.7 + Math.random() * 0.6) * rep),
      ),
    });
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSheeps((prev) => {
        ...
        prev.forEach((s) => {
          const passive = sheepConfig[s.type].passive;

          if (!passive || s.isBald) return;

          if (passive.type === "groupHeal") {
            const heal =
              Math.floor(
                Math.random() * (passive.maxHeal - passive.minHeal + 1),
              ) + passive.minHeal;

            updated = updated.map((sh) => ({
              ...sh,
              hunger: Math.min(100, sh.hunger + heal),
            }));
          }

          if (passive.type === "timeWarp") {
            setTimeRipple(true);
    ...
    const interval = setInterval(() => {
      setSheeps((prevSheeps) => {
        const afterHunger = prevSheeps.map((s) => {
          ...
          if (timePhase === "night" && s.isBald && s.type !== "penguin")
            hungerLoss += 1;
  ...
  const getSheepLine = (s: {
    ...
    if (s.type === "penguin") {
      return randomFrom(penguinLines);
    }
  ...
  useEffect(() => {
    if (page === "catalogue") {
      const newFlavor: Record<SheepType, string> = {
        ...
        penguin: randomFrom(sheepCatalogueText.penguin),
        time: randomFrom(sheepCatalogueText.time),
      };
  ...
  return (
    <div className={`gameRoot ${currentEvent === "famine" ? "famine" : ""}`}>
      {timeRipple && <div className="timeRipple" />}
      ...
      {marketOpen && (
        <div className="market">
          ...
          <p>Penguin Wool Price: ${woolMarket.penguin}</p>
          <button onClick={() => sellWool("penguin")}>
            Sell Penguin Wool(???)
          </button>
        </div>
      )}
      ...
      >
        ...
        <p>Penguin: {woolInventory.penguin}</p>
      </div>
